...
def train_model():
    ...
    data["Subiect"] = data["Subiect"].apply(wordopt)
    print("Dataset text is clear and names got extracted!")

    print("Cautarea cuvintelor cheie...")
    ...
    # Ensure 'Cuvinte_Cheie' is present before splitting
    feature_columns = [
        "Text",
        "Publicatie",
        "Titlu",
    ...
    report_lr = classification_report(y_test, adjusted_pred_lr, output_dict=True)
    ...
    report_dt = classification_report(y_test, adjusted_pred_dt, output_dict=True)
    ...
    report_gbc = classification_report(y_test, adjusted_pred_gbc, output_dict=True)
    ...
    report_rfc = classification_report(y_test, adjusted_pred_rfc, output_dict=True)
...
def manual_testing(text, publication, title, category, models, vectorizers, accuracies):
    ...
    explanations = {
        "Logistic Regression": "Un model statistic care estimează probabilitea unui rezultat binar. Simplu şi eficient pentru sarcini de clasificări binare (în cazul nostru, Adevărat sau Fals).",
        "Decision Tree": "Model ce foloseşte un grafic de decizii arborescent şi posibilele lor consecinţe în predicţie. Uşor de interpretat, dar pot ajunge la supra-umplere.",
